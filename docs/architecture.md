...
| Component            | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AuthLayout`         | `pageBg` (full-viewport, flex-centered, decorative gradient) wraps a single `container` card sized to its content, not stretched — both pages vertically center the same way regardless of form height. Two columns inside the card: left = logo + heading + Clerk form slot + switch-account link + back-to-home link; right = `FeatureCarousel`, hidden below 968px. Fade+up entrance on the container (Framer Motion, `useReducedMotion()`-gated).                                               |
...
| `clerkAppearance.ts` | Shared Clerk `appearance` config: `variables` map Clerk's theme vars to our CSS custom properties (color, radius, font); `elements.header`/`elements.footer` are hidden — the page renders its own heading and switch-account link instead, styled with our type scale. `elements.rootBox`/`cardBox` are forced to `width: 100%` — Clerk's `.cl-rootBox` otherwise renders at its own intrinsic width regardless of parent width, which silently overflowed on mobile (see session-log 2026-09-04). |
...
**Grid convention:** use `minmax(0, 1fr)` rather than plain `1fr` for `grid-template-columns` on this project — a plain `1fr` track won't shrink below its content's min-content width, which caused a silent mobile overflow once already (clipped invisibly by a parent's `overflow: hidden` rather than showing a scrollbar).
