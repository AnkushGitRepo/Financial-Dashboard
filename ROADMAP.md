...
### Phase 10a — retrieval plumbing + grounded chat & insights 🔄 code-complete on `phase-10-rag`

_13/14 done — all code + docs + tests (247 web / 93 api green). Only the prod deploy + merge is left, and that needs sign-off. Branch: `phase-10-rag`._
...
- [ ] **BLOCKER — retrieval doesn't run on the hosted instance.** `onnxruntime-node` can't load `libonnxruntime.so.1` in Vercel's serverless runtime; `outputFileTracingIncludes` for the `.so` blew past the Hobby plan's 12-function limit (reverted, `e5529cb`). Hosted `retrieve()` always returns `null` → chat/insights = pre-Phase-10 behaviour (graceful, no 500s). **Self-host has full RAG.** User picked **the WASM backend** as the fix direction, but it is **not a config tweak** (investigated 2026-09-06): `@huggingface/transformers` is a Next built-in `serverExternalPackages` entry so its `require('onnxruntime-node')` runs at runtime uncontrolled by bundler aliases; `onnxruntime-node/binding.js` loads the `.node` addon at module-eval (so the `globalThis[Symbol.for("onnxruntime")]` hatch can't intercept); an npm `overrides` redirect to `onnxruntime-web` still hits transformers.js's Node branch hardcoding `defaultDevices=['cpu']` with no `wasm` in `supportedDevices`, and forcing the web branch needs faking `IS_NODE_ENV`. It's a multi-hour spike, not one line. Alternatives still on the table: a small separate embedding service on the fundamentals-api (Python), or Vercel Pro. See memory `onnxruntime-vercel-blocker`.
- [ ] **After the blocker:** `POST /api/cron/index-corpus?indexesOnly=1` to create the Atlas Vector Search index, a first full indexer run, confirm `CRON_SECRET` repo secret covers `index-corpus.yml` (it reuses the existing one), README self-host note.
