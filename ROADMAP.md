...
- [ ] **Prod deploy + merge** — needs sign-off. On approval: merge `phase-10-rag` → `main` (+ `v2`), deploy both Vercel projects, run `POST /api/cron/index-corpus?indexesOnly=1` once to create the Atlas Vector Search index, then a full indexer run, add the `CRON_SECRET` repo secret so `index-corpus.yml` fires. Self-host note in README.
