...
**Mitra chat (part 5):**
- [ ] **pt.5:** replace `aiWidgetContent.ts`'s scripted `AI_REPLIES` with `POST /api/ai/chat` — streamed (AI SDK), context = portfolio summary + holdings + recent news, requires the user's key, declines advice-seeking questions. Wire `AiWidget.tsx` to it.
