...
- [ ] **Phase 10a archiving protocol** — collapse the "Retrieval (RAG)" section in `architecture.md` to a summary + `docs/archive/rag-chat.md`; roll old session-log entries; update `CLAUDE.md` Active focus.
- [ ] **Follow-ups (non-blocking):** README self-host note for the RAG env (`FUNDAMENTALS_API_URL` reachable from the app, `IPO_INGEST_TOKEN` shared); pre-bundle the embedding model to kill the ~18s cold-instance download; filings-in-corpus needs an un-blocked PDF host (or a DRHP-URL source); a "clear chat history" control in the widget.
