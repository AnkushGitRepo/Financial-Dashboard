...
- [ ] **Corpus indexer (filings)** — extend the indexer to pull `GET /companies/{symbol}/documents` for a symbol set + `POST /documents/extract-text`, then `indexTextDocument(source, {docType:'filing',…}, text)`. Reuses everything above; just the filing-source loop + a symbol list (holdings-derived or a curated Nifty set).
