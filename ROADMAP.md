...
- [x] **Per-user corpus sync** — `src/lib/rag/userSync.ts`: `syncUserNote` / `removeUserNote` / `syncUserHoldings` — chunk + `embedBatch` + `replaceSourceChunks` under the caller's `userId` (`note:<id>`, `holdings:<userId>`). Every function swallows its errors and returns a boolean — a sync failure never fails the originating write. Called fire-and-forget (`void`) from the notes routes. 8 tests. _(Wiring `syncUserHoldings` into the holdings mutation routes: pending, same one-liner pattern.)_
- [~] **`userNotes` collection + CRUD** — `src/lib/notes/userNotes.ts` (`userNotes` collection: title/body/optional symbol, trim+truncate, 200/user cap) + `GET|POST /api/notes` + `PATCH|DELETE /api/notes/{id}` (owner-scoped, fire-and-forget corpus sync). `public/openapi.json` entries added. 8 tests. **Panel UI still to build** (design-system notes list + editor, a `/dashboard/notes` page + nav item).
